...
def expand_events(year: int, rules: List[RecurringRule], all_groups: List[str], start: date | None = None, end: date | None = None) -> List[Event]:
    events: List[Event] = []
    for rule in rules:
        dates = rule.generate_dates(year, start=start, end=end)
...
def assign_leaders(events: List[Event], leaders: List[Leader], strategy_name: str = "round_robin", leaders_per_event: int = 2) -> List[Assignment]:
    ...
    for ev in events:
        events_by_month.setdefault((ev.date.year, ev.date.month), []).append(ev)
        ...
        for ev in evs_sorted:
            if ev.responsibility_mode == "group" and ev.rotation_pool:
                ...
                for g in ev.rotation_pool:
                    candidates.append((g in month_used, global_usage.get(g, 0), g))
    ...
    for ev in events:
        if not ev.leader_required:
            assignments.append(Assignment(event=ev, leaders=[], responsible_group=ev.responsible_group))
            continue
        if ev.kind == "combined":
            # If leader mode: exactly one leader, else allow configured count (fallback to 0 if none required)
            eligible = [ld for ld in leaders if any(g in ev.groups_involved for g in ld.groups) and ld.is_available_on(ev.date)]
            ...
            assignments.append(Assignment(event=ev, leaders=chosen, responsible_group=ev.responsible_group))
            ...
            for ld in group_to_leader.values():
                if ld.name not in seen:
                    unique.append(ld)
                    seen.add(ld.name)
            assignments.append(Assignment(event=ev, leaders=unique, responsible_group=ev.responsible_group))
    return assignments


def build_schedule(year: int, leaders_raw, groups_raw, rules_objs: List[RecurringRule], start: date | None = None, end: date | None = None, strategy: str = "round_robin") -> Schedule:
    leaders = build_leaders(leaders_raw)
    groups = build_groups(groups_raw)
    all_group_names = list(groups.keys())
    events = expand_events(year, rules_objs, all_group_names, start=start, end=end)
