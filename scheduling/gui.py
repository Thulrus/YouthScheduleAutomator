...
class SchedulerGUI:
    def __init__(self, root: tk.Tk):
        ...
        # Row 0 timezone
        ttk.Label(frm, text="Timezone:").grid(row=0, column=0, sticky="e", **pad)
        self.tz_var = tk.StringVar(value="America/Denver")
        ttk.Entry(frm, textvariable=self.tz_var, width=20).grid(row=0, column=1, sticky="w", **pad)

        # Row 1 Start/End selectors
        ...
        # Row 2 Leaders per combined
        ttk.Label(frm, text="Leaders / Combined:").grid(row=2, column=0, sticky="e", **pad)
        self.leaders_per_var = tk.IntVar(value=2)
        ttk.Spinbox(frm, from_=1, to=5, textvariable=self.leaders_per_var, width=6).grid(row=2, column=1, sticky="w", **pad)
    ...
    def _on_generate(self):
        # Start/end date composed from selector widgets
