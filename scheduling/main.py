...
def parse_args():
    ...
    p.add_argument("--strategy", type=str, default="round_robin", choices=["round_robin", "random", "weighted"]) 
    p.add_argument("--leaders-per-event", type=int, default=2)
    p.add_argument("--out", nargs="+", default=["md"], help="Output formats: md csv ics")
...
def main():
    ...
    schedule = build_schedule(args.year, raw.leaders, raw.groups, rules, start=start, end=end, strategy=args.strategy)
    ...
    if "ics" in args.out:
        exporters.write_ics(schedule, out_dir / "schedule.ics")
